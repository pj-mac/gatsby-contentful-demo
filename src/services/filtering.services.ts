...
function filterConsultants(consultants: IConsultant[], params: IConsultantForm) {
  ...
  if (arr.length === 0 && params?.skills?.length === 0) {
    const filteredNameStartsWith = consultants
      .filter((consultant) => consultant.name.toLowerCase().startsWith(params.name.toLowerCase()));

    const filteredNameIncludes = consultants
      .filter((consultant) => consultant.name.toLowerCase().includes(params.name.toLowerCase()));
    ...
    const noDuplicates = allFiltered
      .reduce((acc: IConsultant[], currentValue: IConsultant) => {
